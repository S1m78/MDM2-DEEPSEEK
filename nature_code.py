...
import torch.mps
torch.mps.empty_cache()  # Clears Metal cache
...
def energy_fn(inputs):
    return torch.sum(0.5 * (inputs['mu'] - inputs['x'])**2)

# Define a simplified predictive coding model
class PredictiveCodingNet(nn.Module):
    def __init__(self, input_size=28*28, hidden_size=64, output_size=10):
        ...
        self.pc1 = pc_layer.PCLayer(energy_fn=energy_fn)
        self.fc2 = nn.Linear(hidden_size, output_size)
        self.pc2 = pc_layer.PCLayer(energy_fn=energy_fn)
...
train_loader = DataLoader(train_dataset, batch_size=32, shuffle=True)  # Reduce batch size for speed
test_loader = DataLoader(test_dataset, batch_size=32, shuffle=False)

# Initialize model and trainer
model = PredictiveCodingNet().to(device)
trainer = PCTrainer(model=model, optimizer_x_fn=optim.AdamW, optimizer_x_kwargs={"lr": 0.001}, T=10)  # Reduce T
...
for epoch in range(num_epochs):
    for images, labels in train_loader:
        ...
        trainer.train_on_batch(inputs=images, loss_fn=nn.CrossEntropyLoss(), loss_fn_kwargs={'target': labels})
